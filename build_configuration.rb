...
cmd = `ls #{File.expand_path(File.join(File.dirname(__FILE__), "resources"))}`
cmd.split("\n").each { |fname| resources << "resources/#{fname}" }

configuration do |c|
	# Undocumented option 'project_name'
	# default value: "BioCatalogue-Mass-Curator"
	#
	#c.project_name = "BioCatalogue-Mass-Curator"
	...
	# default value: []
	#
	c.files_to_copy = [resources].flatten
	...
	# default value: nil
	#
	#c.mac_do_not_generate_plist = 1

	# Undocumented option 'mac_icon_path'
	# default value: "GenericJavaApp.icns"
	#
	c.mac_icon_path = "resources/biocatalogue.icns"
